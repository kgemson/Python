...
import backend_postgre as be

#################################################################################################################################
#define functions to link frontend to backend
def view_command():
    list1.delete(0,END)
    for r in be.view_all():
...
def search_command():
    list1.delete(0,END)
    rows_found=be.search_entry(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()) #'get' required because 'StringVar' != String
    if len(rows_found)==0:
...
def add_command():
    be.add_entry(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()) #'get' required because 'StringVar' != String
...
def update_command():
    ...
    be.update_entry(my_id,title_text.get(),author_text.get(),year_text.get(),isbn_text.get()) #'get' required because 'StringVar' != String
